...
class Beam:
    def __init__(
        ...
        self.prev_ks = []
        self.next_ys = [np.full(beam_size, start_token_id)]  # remove padding

        self.current_scores = np.zeros(beam_size, dtype=np.float32)
        self.all_scores = []
    ...
    def advance(self, next_log_probs):
        ...
        vocabulary_size = next_log_probs.shape[1]
        # current_beam_size = next_log_probs.shape[0]

        current_length = len(self.next_ys)
        if current_length < self.min_length:
            for beam_index in range(next_log_probs.shape[0]):
                next_log_probs[beam_index, self.end_token_id] = -1e10

        if len(self.prev_ks) > 0:
            beam_scores = next_log_probs + np.expand_dims(
                self.current_scores, 1
            ).repeat(vocabulary_size, axis=1)
            # Don't let EOS have children.
            last_y = self.next_ys[-1]
            for beam_index in range(last_y.shape[0]):
        ...
        flat_beam_scores = beam_scores.flatten()
        top_score_ids, top_scores = self.topk_by_partition(
            flat_beam_scores, k=self.beam_size, axis=0, ascending=False
        )
    ...
    def get_current_state(self):
        ...
        return np.stack(self.next_ys, axis=1)
